...
public class NetworkTask extends AsyncTask<Integer, String, Object> {
    ...
    ArrayList<JsonBean> beans; // JsonMember는 Bean // 얘는 리턴 값있고, 파서는 없다.
    ...
    public NetworkTask(Context context, String mAddr){
        ...
        this.beans = new ArrayList<JsonBean>();
    }
    ...
    private void parser(String str){
        ...
        try{
            ...
            //members info 임을 체크
            JSONArray jsonArray = new JSONArray(jsonObject.getString("recycle_cardwebview"));
            //혹시 하다가 다른 json 눌렀을때 같이 보일까봐.(파싱전에 아무데나! 사용)
            beans.clear(); //arraylist 지울려고 사용
            ...
            for(int i=0; i<jsonArray.length(); i++) {
                ...
                //가져온걸 bean에다가 정의
                JsonBean bean = new JsonBean(name,image,info); //bean으로 받아서
                beans.add(bean);//어레이리스트에 넣어줌
